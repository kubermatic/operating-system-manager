...
func TestReconciler_Reconcile(t *testing.T) {
	testCases := []struct {
		...
		{
			name:                   "Ubuntu OS in OpenStack with Containerd",
			ospFile:                defaultOSPPathPrefix + fmt.Sprintf("%s.yaml", ospUbuntu),
			ospName:                ospUbuntu,
			operatingSystem:        providerconfig.OperatingSystemUbuntu,
			oscFile:                "osc-ubuntu-openstack-containerd.yaml",
			mdName:                 "ubuntu-openstack",
			kubeletVersion:         defaultKubeletVersion,
			provisioningSecretFile: "secret-ubuntu-openstack-containerd-provisioning.yaml",
			bootstrapSecretFile:    "secret-ubuntu-openstack-containerd-bootstrap.yaml",
			config: testConfig{
				namespace:             "kube-system",
				containerRuntime:      "containerd",
				externalCloudProvider: true,
			...
			cloudProviderSpec: runtime.RawExtension{Raw: []byte(`{}`)},
		},
		{
