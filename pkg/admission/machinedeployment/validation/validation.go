...
func (h *AdmissionHandler) Handle(ctx context.Context, req webhook.AdmissionRequest) webhook.AdmissionResponse {
	...
	case admissionv1.Update:
		if err := h.decoder.Decode(req, md); err != nil {
			return webhook.Errored(http.StatusBadRequest, fmt.Errorf("error occurred while decoding machinedeployment: %w", err))
		}
		allErrs = append(allErrs, h.validateMachineDeployments(ctx, *md)...)

	case admissionv1.Delete:
		// NOP we don't need validations for delete operations
		return webhook.Allowed(fmt.Sprintf("machinedeployment validation request %s allowed; validation is not required for delete request", req.UID))

	default:
		return webhook.Errored(http.StatusBadRequest, fmt.Errorf("%s not supported on machinedeployment resources", req.Operation))
	}
