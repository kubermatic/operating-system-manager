...
import (
	...
	"k8c.io/operating-system-manager/pkg/util/kubeconfig"
...
func (b *Bootstrap) CreateBootstrapKubeconfig(ctx context.Context, name string) (*clientcmdapi.Config, string, error) {
	...
	outConfig, err := b.generateKubeconfig(ctx, token)
	if err != nil {
		return nil, "", err
	}

	configString, err := kubeconfig.StringifyKubeconfig(outConfig)
	if err != nil {
		return nil, "", err
	}
		...
		if kerrors.IsNotFound(err) {
			bootstrapConfig := &corev1.Secret{
				...
				Data: map[string][]byte{
					"kubeconfig": []byte(configString),
				},
			}

			return outConfig, bootstrapConfigName, b.client.Create(ctx, bootstrapConfig)
		}

		return nil, "", err
	}

	// We check here if the token expiration time has been updated to decide whether we need to generate a new token and
	// updated the secret with the new token. If the expiration time has been updated, then nothing to be done.
	if updated {
		return outConfig, bootstrapConfigName, nil
	}

	bootstrapConfig.Data = map[string][]byte{
		"kubeconfig": []byte(configString),
	}

	err = b.client.Update(ctx, bootstrapConfig)
	if err != nil {
		return nil, "", err
	}

	return outConfig, bootstrapConfigName, err
}

func (b *Bootstrap) generateKubeconfig(ctx context.Context, token string) (*clientcmdapi.Config, error) {
